import React, {Fragment} from 'react'
import MetaData from '../layout/MetaData'
import Sidebar from './Sidebar'

const UpdateLabor = () => {
  return (
    <Fragment>
    <MetaData title={'Update Labor'} />
    <div className="row">
        <div className="col-12 col-md-2">
            <Sidebar />
        </div>

        <div className="col-12 col-md-10">
            <Fragment>
            <div className="wrapper my-5"> 
  <form className="shadow-lg" encType="multipart/form-data">
    <h1 className="mb-4">Update Labor</h1>
    <div className="form-group">
      <label htmlFor="name_field">Name</label>
      <input type="text" id="name_field" className="form-control"  />
    </div>
    <div className="form-group">
      <label htmlFor="price_field">CNIC</label>
      <input type="text" id="price_field" className="form-control"  />
    </div>
    <div className="form-group">
      <label htmlFor="description_field">Work Description</label>
      <textarea className="form-control" id="description_field" rows={2} defaultValue={""} />
    </div>
    <div className="form-group">
      <label htmlFor="category_field">Type</label>
      <select className="form-control" id="category_field">
        <option>Plumber</option>
        <option>Electration</option>
        <option>Civil Engineer</option>
        <option>Painter</option>
        <option>Other</option>
      </select>
    </div>
    <div className="form-group">
      <label htmlFor="stock_field">Contact Number</label>
      <input type="number" id="stock_field" className="form-control"  />
    </div>
    <div className="form-group">
      <label htmlFor="seller_field">Experienced</label>
      <input type="text" id="seller_field" className="form-control"  />
    </div>
    <div className="form-group">
      <label>Images</label>
      <div className="custom-file">
        <input type="file" name="product_images" className="custom-file-input" id="customFile" multiple />
        <label className="custom-file-label" htmlFor="customFile">
          Choose Images
        </label>
      </div>
    </div>
    <button id="login_button" type="submit" className="btn btn-block py-2">
      UPDATE
    </button>
  </form>
</div>

            </Fragment>
        </div>
    </div>

</Fragment>
)
}


export default UpdateLabor
